...
int diamodel3(double x, double ener[3], double grad[3])
{
    double y;
    ...
    for(ip = -3.5; ip <= 0.2; ip=ip+0.1 ) {
        ...
        for(i=0;i<50;i++) {
            ...
            while(fabs(x)<=25 && istep<1000000) {
                ...
                fprintf(fpt,"%12.9f %12.9f %12.9f %12.9f %12.9f %12.9f %12.9f %d ", x, v, e0, e1, creal(rho[0]), creal(rho[1]), cimag(rho[1]), istate);
